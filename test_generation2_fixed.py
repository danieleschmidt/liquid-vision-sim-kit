...
from liquid_vision.simulation.event_simulator import create_simulator
...
def test_liquid_network_robustness():
    ...
    try:
        ...
        model = create_liquid_net(input_dim=2, output_dim=10, architecture="small")
...
def test_event_simulation_robustness():
    ...
    try:
        ...
        simulator = create_simulator(simulator_type="dvs", resolution=(320, 240), contrast_threshold=0.1)
...
def test_training_robustness():
    ...
    try:
        ...
        config = TrainingConfig(epochs=2, batch_size=16, learning_rate=0.001)
...
def test_integration_robustness():
    ...
    try:
        ...
        print(f"✅ End-to-end pipeline: {total_events} events -> model output {output.shape}")
...
def test_error_handling_robustness():
    ...
    try:
        ...
        try:
            invalid_model = create_liquid_net(input_dim=0, output_dim=10, architecture="invalid")
        except (ValueError, KeyError) as e:
            print(f"✅ Invalid architecture caught: {e}")
        
        # Test negative dimensions  
        try:
            negative_model = create_liquid_net(input_dim=-1, output_dim=5, architecture="tiny")
        except Exception as e:  # Catch any error type
            print(f"✅ Negative input dimension caught: {type(e).__name__}: {e}")
        
        # Test simulator edge cases
        try:
            bad_simulator = DVSSimulator(resolution=(0, 0))  # Should handle gracefully
            print(f"⚠️  Zero resolution allowed: {bad_simulator.resolution}")
        except Exception as e:
            print(f"✅ Zero resolution caught: {e}")
